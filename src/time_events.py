"""
Time-based event scheduling utility that supports callbacks for sunup,
sundown, and custom times based on geographic location and date.
"""
...
class TimeEvents(BaseComponent):
    """
    Schedules callbacks to be fired at specific time events (e.g., sunup,
    sundown) and delays.

    Attributes:
        config (dict): Configuration settings for time events, including
                       location details.
        location (LocationInfo): The geographic location for calculating
        sunup and sundown times.
    """
    ...
    def _calculate_event_time(self, event: str) -> datetime:
        """
        Determines the time of the specified event (sunup, sundown) based on
    ...
    def schedule_sunup_callback(self, callback, delay_seconds: float = 0) -> None:
        """
        Schedules a callback to be fired after the sunup time and the specified
        delay.

        Args:
            callback (callable): The function to call after the delay.
            delay_seconds (float): The delay in seconds after the event to
            wait before firing the callback.
        """
    ...
    def schedule_sundown_callback(self, callback, delay_seconds: float = 0) -> None:
        """
        Schedules a callback to be fired after the sundown time and the
        specified delay.
    ...
    def _schedule_recurring_event(self, event: str, callback, 
                                  delay_seconds: float) -> None:
        """
        Schedules a callback to be fired at the specified event time and
        reschedules it for the next day.
        ...
        self.log_debug(
            f"Scheduling callback in {total_delay} seconds for event '{event}'"
            )

        def delayed_execution():
            callback()  # Execute the callback
            self.log_debug(
                f"Callback for event {event} executed, rescheduling for next day"
                )
    ...
    def _schedule_test_event(self, event: str, 
                             callback, delay_seconds: float) -> None:
        """
        Schedules a callback to be fired at the specified event time and
        reschedules it for the next day.
        ...
        def delayed_execution():
            try:
                if callable(callback):
                    ...
                    self.log_error(
                        f"Callback for event '{event}' is not callable"
                        )
    ...
    def get_timezone(self) -> str:
        ...
        self.log_debug(
            f"Timezone for location '{self.location.name}': {timezone}"
            )
        return timezone
